...
ALTER TABLE Projects ADD UpdatedAt NVARCHAR(100);
...
CREATE  PROCEDURE [dbo].[sp_AddProjectTeamMember]
...
CREATE  PROCEDURE [dbo].[sp_RemoveProjectTeamMember]
    @ProjectId NVARCHAR(50),
    @MemId NVARCHAR(20)
AS
BEGIN
...
CREATE  PROCEDURE [dbo].[sp_GetProjectsByTeamMember]
    @MemId NVARCHAR(20)
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE  PROCEDURE [dbo].[sp_GetProjectTeamMembers]
    @ProjectId NVARCHAR(50)
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE  PROCEDURE [dbo].[sp_AddProjectService]
    @ProjectId NVARCHAR(50),
...
CREATE  PROCEDURE [dbo].[sp_RemoveProjectService]
    @ServiceId INT
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE  PROCEDURE [dbo].[sp_GetServicesByProject]
    @ProjectId NVARCHAR(50)
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE  PROCEDURE [dbo].[sp_GetActiveProjectsDashboard]
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE  PROCEDURE [dbo].[sp_GetProjectSummary]
    @ProjectId NVARCHAR(50)
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT 
        COUNT(*) AS TeamSize,
        STRING_AGG(tm.Name + ' ' + tm.Name, ', ') AS TeamMembers
    FROM ProjectTeamMembers ptm
    INNER JOIN TeamMembers tm ON ptm.MemId = tm.MemId
    WHERE ptm.ProjectId = @ProjectId;
END
...
CREATE  PROCEDURE [dbo].[sp_GetProjectTimeline]
    @ProjectId NVARCHAR(50)
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT 
        'Team Member Added: ' + tm.Name + ' ' + tm.Name AS Event,
...
CREATE  PROCEDURE [dbo].[sp_ValidateProjectData]
    @ProjectName NVARCHAR(100),
    @ClientId NVARCHAR(50),
    @StartDate DATETIME2,
    @EndDate DATETIME2,
